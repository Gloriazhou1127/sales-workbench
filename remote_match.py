...
# 获取所有需要匹配的填报记录，按 filled_by 分组，组内按 filled_at 排序
unmatched_logs = cursor.execute("""
    SELECT id, hardware_id as old_hid, filled_by, fill_date, filled_at
    FROM key_account_followup_logs
    WHERE crm_account_id IS NULL
    ORDER BY filled_by, filled_at
""").fetchall()

# 获取每个填报人（按姓名匹配）对应的客户列表（按 classroom_count DESC 排序）
# owner_name 格式：姓名-工号，需要提取姓名部分匹配 filled_by
owner_customers = {}
...
# 按 classroom_count DESC 排序每个填报人的客户列表
    ...
    for i, log in enumerate(logs):
        if i < len(customers):
            ...
            match_results.append({
                ...
                'old_hardware_id': log['old_hardware_id'],
...
# 打印预览（前50条）
print("log_id | filled_by | fill_date   | 匹配到的客户 | customer_name | crm_account_id")
print("-" * 100)
for r in match_results[:50]:
    print("%6d | %8s | %10s | ID=%-6d | %s | %s" % (
...
print()
print("CSV已保存到: /opt/sales-workbench/match_preview.csv")
